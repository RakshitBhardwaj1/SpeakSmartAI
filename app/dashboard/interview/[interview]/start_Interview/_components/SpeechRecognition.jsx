  ...
  const finalizeCurrentAnswer = async () => {
    ...
    try {
      ...
      if (finalAnswer.length >= 10) {
        // Wait up to 1 second for the audio blob to be generated by mediaRecorder.onstop
        let audioBlob = null;
        for (let i = 0; i < 20; i++) {
        ...
        if (audioBlob) {
          try {
            ...
            if (response.ok) {
              ...
              feedbackResult = {
                rating: analysisData.report_card?.overall_score || 0,
                feedback: analysisData.feedback || "Audio analyzed successfully.",
                strengths: `Prosody Score: ${analysisData.report_card?.overall_score}/100. Pacing: ${analysisData.report_card?.pacing?.score}. Tone/Expressiveness: ${analysisData.report_card?.expressiveness?.score}`,
                modelAnswer: `Metrics: ${JSON.stringify(analysisData.report_card)}`,
                detailedAnalysis: analysisData
              };
              console.log("Python API analysis successful");
            } else {
              const errorText = await response.text();
              toast.error('Speech analysis API error: ' + errorText);
              throw new Error('Speech analysis API error: ' + errorText);
            }
          } catch (e) {
            toast.error('Could not connect to speech-analysis-api. Please try again.');
            console.error("Speech analysis API error:", e);
            return;
          }
        } else {
